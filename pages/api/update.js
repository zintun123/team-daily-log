...
export default async function handler(req, res) {
  ...
  try {
    const eodRows = tasks.map((t, i) => ({
      date, name, time_in: "", time_out: timeOut,
    ...
    const newTaskRows = newTasks.map((t, i) => ({
      date, name, time_in: "", time_out: timeOut,
      task_no: tasks.length + i + 1, task_name: t.name, category: t.category, phase: t.phase,
      pct: t.pct === "" ? null : t.pct, assigned_by: t.assignedBy, remarks: t.remarks,
      type: "EOD"
    }));
    ...
    if (allRows.length === 0) return res.status(200).json({ ok: true, appended: 0 });
